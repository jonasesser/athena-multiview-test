...
const ComponentName = ExampleWebViewEvents.ViewName1;
export default defineComponent({
    ...
    mounted() {
        WebViewEvents.on(ExampleWebViewEvents.ServerToWebView1.REFRESH_PLAYERS, this.loadPlayers);
        WebViewEvents.on(ExampleWebViewEvents.ClientToWebView1.LOAD_PLAYERS, this.loadPlayers);
        WebViewEvents.emitReady(ExampleWebViewEvents.ViewName1);
    ...
    methods: {
        close() {
            ...
            WebViewEvents.emitClient(ExampleWebViewEvents.ClientServer1.CLOSE);
        },
        loadPlayers(players: ExPlayerInfo[]) {
            this.players = players;
        },
        kick(id: number) {
            WebViewEvents.emitServer(ExampleWebViewEvents.WebViewToServer1.KICK_PLAYER, id);
        },
        refresh() {
            WebViewEvents.emitServer(ExampleWebViewEvents.WebViewToServer1.REQUEST_REFRESH);
        },
        getOpenTime() {
